...
class Dataset(paddle.io.Dataset):
    def __init__(self, dataset_root=None, transforms=None, mode = "train", patch_size=256):
        if dataset_root is None:
            raise ValueError("dataset_root is None")
        self.dataset_root = dataset_root
        if transforms is not None:
            self.transforms = Compose(transforms, patch_size=patch_size)
        else:
            self.transforms = None
        self.mode = mode
        
        self.im1_list = [path.strip() for path in open(self.dataset_root, 'r').readlines()]
        self.im2_list = [path.replace("blur_image", "gt_image") for path in self.im1_list]
